WITH Input AS (
	SELECT MyTable.x
	FROM (
		VALUES ('17-19 p: pwpzpfbrcpppjppbmppp'),
...
SplitInput AS (
	SELECT
		CAST(LEFT(Input.x, CHARINDEX('-', Input.x) - 1) AS INT) AS MinCount,
		CAST(SUBSTRING(Input.x, CHARINDEX('-', Input.x) + 1, CHARINDEX(':', Input.x) - CHARINDEX('-', Input.x) - 2) AS INT) AS MaxCount,
		SUBSTRING(Input.x, CHARINDEX(':', Input.x) - 1, 1) AS Letter,
		SUBSTRING(Input.x, CHARINDEX(':', Input.x) + 2, LEN(Input.x)) AS Password
	FROM Input
),
SplitWithMagicChars AS (
	SELECT
		*,
		SUBSTRING(SplitInput.Password, SplitInput.MinCount, 1) AS MinChar,
		SUBSTRING(SplitInput.Password, SplitInput.MaxCount, 1) AS MaxChar
	FROM SplitInput
)
SELECT COUNT(*) AS Answer
FROM SplitWithMagicChars
WHERE
	(
	SplitWithMagicChars.MinChar = SplitWithMagicChars.Letter
		OR SplitWithMagicChars.MaxChar = SplitWithMagicChars.Letter
)
	AND SplitWithMagicChars.MinChar <> SplitWithMagicChars.MaxChar