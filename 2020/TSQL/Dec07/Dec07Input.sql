USE tempdb
GO
CREATE OR ALTER VIEW dbo.Dec07Input
AS
WITH Input AS (
	SELECT 'dotted blue bags contain 5 wavy green bags, 3 pale beige bags.
...
bright yellow bags contain no other bags.' AS InputString
),
InputLines AS (
	SELECT
		ROW_NUMBER() OVER (ORDER BY Input.InputString) AS GroupId,
		REPLACE(REPLACE(value, ' bags contain ', '^'), ', ', '|') AS ItemString
	FROM Input
	CROSS APPLY STRING_SPLIT(REPLACE(REPLACE(Input.InputString, '.', ''), CHAR(0xD) + CHAR(0xA), '^'), '^')
)
SELECT
	InputLines.*,
	LEFT(InputLines.ItemString, CHARINDEX('^', InputLines.ItemString) - 1) AS BagColor,
	LEFT(ContainsString.value, CHARINDEX(' ', ContainsString.value) - 1) AS ContainsBagCount,
	REPLACE(REPLACE(SUBSTRING(ContainsString.value, CHARINDEX(' ', ContainsString.value) + 1, LEN(ContainsString.value)), ' bags',''),' bag','') AS ContainsBagColor
FROM InputLines
CROSS APPLY STRING_SPLIT(SUBSTRING(InputLines.ItemString, CHARINDEX('^', InputLines.ItemString) + 1, LEN(InputLines.ItemString)), '|') AS ContainsString
GO
