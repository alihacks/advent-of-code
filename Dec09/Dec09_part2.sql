...
AnswerIndexes AS (
	SELECT
		RunningSums.StartIndex,
		RunningSums.EndIndex
	FROM RunningSums
	WHERE RunningSums.RangeSum = (SELECT Part1Answer.Answer FROM Part1Answer)
)
SELECT MIN(I.Num) + MAX(I.Num) AS Answer
FROM AnswerIndexes AS A
CROSS APPLY #input AS I
WHERE
	I.NumIndex BETWEEN A.StartIndex AND A.EndIndex
